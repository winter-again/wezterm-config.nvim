...
---@param name string
local function is_sup_profile(name)
    local sup_opts = { 'colors', 'background', 'font' }
    for _, opt in ipairs(sup_opts) do
        if opt == name then
            return true
        end
    end
...
function M.override_user_var(overrides, name, value, profile_data)
    -- TODO: find cleaner way to have just one override_... func
    -- and therefore optionally profile_data is optional
    if profile_data ~= nil then
        if is_sup_profile(name) then
            overrides[name] = profile_data[name][value]
            return overrides
        else
            -- this gets printed to Wezterm logs at the INFO level
            -- because print is an alias for wezterm.log_info()
